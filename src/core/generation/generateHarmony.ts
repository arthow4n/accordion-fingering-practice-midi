...
export const generateHarmony = (context:TonalContext,meter:Meter,measures:number,allowed:readonly string[],chordVocabulary:readonly ChordQuality[],rng:Rng,legacyTemplateId?:string): {events:HarmonyEvent[];progressionId:string} => {
  const legacy=legacyBassLineById(legacyTemplateId);const duration=ticksPerMeasure(meter);
  if(legacy){
    if(legacy.meter!==`${meter.beats}/${meter.beatUnit}`)throw new Error(`${legacy.id} requires ${legacy.meter}`);
    if(legacy.fixedKey&&legacy.fixedKey!==`${context.tonic} ${context.mode}`)throw new Error(`${legacy.id} requires ${legacy.fixedKey}`);
    if(measures!==legacy.harmony.length)throw new Error(`${legacy.id} requires exactly ${legacy.harmony.length} measures`);
    const events=legacy.harmony.map((step,index)=>{const quality=step.quality??qualityForDegree(context.mode,step.degree);const fn=step.degree===1||step.degree===6?"tonic":step.degree===5||step.degree===7?"dominant":"predominant";return{id:`harmony-${index}`,onset:index*duration,duration,rootDegree:{degree:step.degree,alteration:0,octaveOffset:0},quality,function:fn,symbol:step.symbol??romanFor(step.degree,quality,false)} satisfies HarmonyEvent;});
    return{events,progressionId:legacy.id};
  }
