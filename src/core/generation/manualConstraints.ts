...
export const manualConstraintViolations=(exercise:Exercise,request:TrainingRequest)=>{
 ...
 for(let index=1;index<bass.length;index++){const jump=Math.abs((bass[index]!.metadata.stradellaColumn??0)-(bass[index-1]!.metadata.stradellaColumn??0));if(jump<request.leftHand.minJump||jump>request.leftHand.maxJump)violations.push(`left-hand jump ${jump} outside ${request.leftHand.minJump}..${request.leftHand.maxJump}`);}
