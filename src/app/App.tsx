...
export default function App(){
 ...
 const finish=useCallback(()=>{const timeline=timelineRef.current;if(!timeline)return;cancelAnimationFrame(frameRef.current??0);const result=evaluatePerformance(exercise,performedRef.current,timeline.sessionStartMs()).metrics;setMetrics(result);const attempts=result.rightHand.attempts+result.leftHand.attempts;const correct=result.rightHand.correct+result.leftHand.correct;setSessionStats(x=>({...x,completedExercises:x.completedExercises+1,completedEvents:x.completedEvents+attempts,attempts:x.attempts+attempts,correct:x.correct+correct,timingCorrect:x.timingCorrect+Math.round(result.timingAccuracy*attempts),missed:x.missed+result.missedNotes,extra:x.extra+result.extraNotes}));regenerate(nextSeed(),true);},[exercise,regenerate]);
 const acceptMidi=useCallback((event:PerformedMidiEvent)=>{if(event.type!=="noteOn")return;if(mode==="correction"){if(status!=="playing")return;const target=expected[correctionIndex];const matches=target&&(!event.hand||event.hand===target.hand)&&target.pitches.some(p=>p.midi===event.midiNote);if(!matches){setSessionStats(x=>({...x,attempts:x.attempts+1}));return;}correctionNotesRef.current.add(event.midiNote);if(target.pitches.every(p=>correctionNotesRef.current.has(p.midi))){correctionNotesRef.current.clear();const next=correctionIndex+1;setSessionStats(x=>({...x,completedEvents:x.completedEvents+1,attempts:x.attempts+1,correct:x.correct+1}));if(next>=expected.length){setSessionStats(x=>({...x,completedExercises:x.completedExercises+1}));regenerate(nextSeed(),true);}else{setCorrectionIndex(next);setPositionMs(ticksToMs(target.onset+target.duration,exercise.tempoBpm));}}return;}if(status==="playing"||status==="countIn")performedRef.current.push(event);},[mode,status,expected,correctionIndex,exercise.tempoBpm,regenerate]);
