...
export default function App(){
 const scoreRef=useRef<HTMLDivElement>(null);const performedRef=useRef<PerformedMidiEvent[]>([]);const timelineRef=useRef<AbsoluteTimeline>();const frameRef=useRef<number>();
 ...
 const expected=useMemo(()=>[...exercise.rightHand,...exercise.leftHand].filter(e=>e.pitches.length).sort((a,b)=>a.onset-b.onset),[exercise]);
 const updateSettings=(next:TrainingRequest)=>{setSettings(next);saveSettings(next);};
 const regenerate=useCallback((newSeed=nextSeed())=>{const next=generateExercise(settings,newSeed);setSeed(newSeed);setExercise(next);setStatus("ready");setMetrics(undefined);setPositionMs(0);setCorrectionIndex(0);performedRef.current=[];console.debug(exerciseDiagnostics(next));},[settings]);
 ...
 const acceptMidi=useCallback((event:PerformedMidiEvent)=>{if(event.type!=="noteOn")return;if(mode==="correction"){const target=expected[correctionIndex];if(target?.pitches.some(p=>p.midi===event.midiNote)){const next=correctionIndex+1;setCorrectionIndex(next);setPositionMs(ticksToMs(target.onset+target.duration,exercise.tempoBpm));if(next>=expected.length)setStatus("finished");}return;}if(status==="playing"||status==="countIn")performedRef.current.push(event);},[mode,expected,correctionIndex,exercise.tempoBpm,status]);
