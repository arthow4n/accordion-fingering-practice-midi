...
export const generateExercise=(raw:TrainingRequest,seed=raw.seed??Date.now(),instrument:InstrumentProfile=accordionProfile):Exercise=>{
 const parsed=parseTrainingRequest(raw); const rng=createRng(seed); const intent:TrainingIntent=parsed.mix?rng.weightedPick(parsed.mix.map(x=>({value:x.intent,weight:x.weight}))):parsed.intent;const request=applyIntentPreset(parsed,intent);
 for(let attempt=1;attempt<=32;attempt++){
  ...
  const phrase=generatePhrasePlan(request.measures,rng); const harmonyResult=generateHarmony(context,meter,request.measures,request.harmony.progressionVocabulary,rng);
  ...
  const validation=validateExercise(exercise,instrument); if(validation.valid)return exercise;
 }
 throw new Error(`Unable to generate a playable exercise after 32 candidates (seed ${seed})`);
};
