...
describe("procedural generator",()=>{
 ...
 it("honors a very slow note frequency with only half and quarter-note durations",()=>{for(const meter of [{beats:4 as const,beatUnit:4 as const},{beats:3 as const,beatUnit:4 as const}]){for(let seed=0;seed<20;seed++){const request=defaultTrainingRequest();request.rhythm.meters=[meter];request.rhythm.smallestSubdivision="quarter";request.rhythm.noteDensity=0;const exercise=generateExercise(request,seed);expect(exercise.rightHand.every(event=>event.duration>=480)).toBe(true);expect(exercise.rightHand.length).toBeLessThanOrEqual(request.measures*meter.beats);}}});
 it("never generates notes shorter than the selected subdivision",()=>{for(const [smallestSubdivision,minimumDuration] of [["quarter",480],["eighth",240],["sixteenth",120]] as const){const request=defaultTrainingRequest();request.intent="rhythmFocus";request.rhythm.smallestSubdivision=smallestSubdivision;request.rhythm.noteDensity=1;for(let seed=0;seed<20;seed++)expect(generateExercise(request,seed).rightHand.every(event=>event.duration>=minimumDuration)).toBe(true);}});
