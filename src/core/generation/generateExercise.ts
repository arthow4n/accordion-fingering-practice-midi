...
const leftJumpProgression=(request:TrainingRequest)=>!request.leftHand.enabled||request.leftHand.jumpFrequency==="none"?request.harmony.progressionVocabulary:[`jump-${request.leftHand.jumpSize}-${request.leftHand.jumpFrequency}`];
export const generateExercise=(raw:TrainingRequest,seed=raw.seed??Date.now(),instrument:InstrumentProfile=accordionProfile):Exercise=>{
 ...
 for(let attempt=1;attempt<=32;attempt++){
  ...
  const phrase=generatePhrasePlan(request.measures,rng); const harmonyResult=generateHarmony(context,meter,request.measures,leftJumpProgression(request),request.harmony.chordVocabulary,rng,request.leftHand.templateId);
  ...
  const validation=validateExercise(exercise,instrument);const manualViolations=manualConstraintViolations(exercise,request);lastRejection=[...validation.errors,...difficultyViolations(exercise.difficulty,difficultyBounds),...manualViolations];if(validation.valid&&satisfiesDifficultyBounds(exercise.difficulty,difficultyBounds)&&!manualViolations.length)return exercise;
 }
