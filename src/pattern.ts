...
export const bassPatterns: BassPattern[] = stradellaBassRows.flatMap((_, index, array) => {
  ...
  const VI = array.at(index + 3) && {
    counter: array[index + 3][0],
    root: array[index + 3][1],
    maj: array[index + 3][2],
    min: array[index + 3][3],
    seventh: array[index + 3][4],
    dim: array[index + 3][5],
  };

  const III = array.at(index + 4) && {
    counter: array[index + 4][0],
    root: array[index + 4][1],
    maj: array[index + 4][2],
    min: array[index + 4][3],
    seventh: array[index + 4][4],
    dim: array[index + 4][5],
  };
  ...
  return compact([
    ...
    // Patterns like
    // Bb Fdim/B F/C D7
    V && II && VI && III && bass(I.root.name, 3, 4, T.PolkaAlt, `16(<${I.root.name}>${I.root.notesString}) 16(${I.maj.notesString}) 16(${I.maj.notesString}) 16(<${V.dim.name}/${VI.counter.name}>${VI.counter.notesString}) 16(${V.dim.notesString}) 16(${V.dim.notesString}) 16(<${V.dim.name}/${VI.root.name}>${VI.root.notesString}) 16(${V.dim.notesString}) 16(${V.dim.notesString}) 16(<${V.dim.name}/${VI.root.name}>${VI.root.notesString}) 16(${V.dim.notesString}) 16(${V.dim.notesString}) 16(<${V.root.name}/${II.root.name}>${II.root.notesString}) 16(${V.maj.notesString}) 16(${V.maj.notesString}) 16(<${III.seventh.name}>${III.root.notesString}) 16(${III.seventh.notesString}) 16(${III.seventh.notesString})`),
