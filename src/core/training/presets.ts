...
export const applyIntentPreset=(request:TrainingRequest,intent:TrainingIntent):TrainingRequest=>{
 ...
 switch(intent){
  ...
  case "rhythmFocus": next.patterns.allowedFamilies=["repeated","scale","chordTone"];next.rhythm.smallestSubdivision="sixteenth";next.rhythm.syncopation=Math.max(.5,next.rhythm.syncopation);next.rhythm.noteDensity=Math.max(.8,next.rhythm.noteDensity);next.coordination.difficulty=Math.max(.5,next.coordination.difficulty);next.rightHand.movementDifficulty=.15;break;
  ...
  case "leftHandFocus": next.patterns.allowedFamilies=["repeated","scale","chordTone"];next.rhythm.smallestSubdivision="quarter";next.leftHand.movementDifficulty=Math.max(.7,next.leftHand.movementDifficulty);next.leftHand.accompanimentStyle="alternatingBass";break;
