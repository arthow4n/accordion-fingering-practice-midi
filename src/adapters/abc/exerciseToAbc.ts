...
const serializeVoice=(events:ExerciseEvent[],exercise:Exercise)=>{
 ...
 for(const event of events){const mi=Math.floor(event.onset/measure);if(mi!==currentMeasure){if(currentMeasure>=0)parts.push("|");currentMeasure=mi;state=new Map();}const duration=event.duration/240;const suffix=duration===1?"":Number.isInteger(duration)?String(duration):`/${Math.round(1/duration)}`;if(!event.pitches.length)parts.push(`z${suffix}`);else if(event.pitches.length===1)parts.push(`${renderPitch(event.pitches[0]!)}${suffix}`);else parts.push(`[${event.pitches.map(renderPitch).join("")}]${suffix}`);}parts.push("|");return parts.join(" ");
};
