...
const serializeVoice=(events:ExerciseEvent[],exercise:Exercise,labels=new Map<number,string>(),markedOnset?:number)=>{
 const signature=signatureMap(exercise.tonalContext), measure=ticksPerMeasure(exercise.meter),beamBeat=exercise.meter.beatUnit===8&&exercise.meter.beats%3===0?720:480*4/exercise.meter.beatUnit;let currentMeasure=-1;let state=new Map<string,string>();const parts:string[]=[];let previous:ExerciseEvent|undefined;
 ...
 for(const event of splitEvents){const mi=Math.floor(event.onset/measure);if(mi!==currentMeasure){if(currentMeasure>=0)parts.push(" |");currentMeasure=mi;state=new Map();previous=undefined;}const duration=event.duration/240;const suffix=duration===1?"":Number.isInteger(duration)?String(duration):`${event.duration/120}/2`;const tie=event.metadata.tieToNext?"-":"";const annotation=labels.get(event.onset);const prefix=`${markedOnset!==undefined&&markedOnset>=event.onset&&markedOnset<event.onset+event.duration?"!mark!":""}${annotation?`"${annotation}"`:""}`;const text=!event.pitches.length?`${prefix}z${suffix}`:event.pitches.length===1?`${prefix}${renderPitch(event.pitches[0]!)}${suffix}${tie}`:`${prefix}[${event.pitches.map(renderPitch).join("")}]${suffix}${tie}`;const beamed=previous&&previous.pitches.length>0&&event.pitches.length>0&&previous.duration<beamBeat&&event.duration<beamBeat&&Math.floor(previous.onset/beamBeat)===Math.floor(event.onset/beamBeat);parts.push(`${beamed?"":" "}${text}`);previous=event;}parts.push(" |");return parts.join("").trim();
};
