...
export const generateExercise=(raw:TrainingRequest,seed=raw.seed??Date.now(),instrument:InstrumentProfile=accordionProfile):Exercise=>{
 ...
 for(let attempt=1;attempt<=32;attempt++){
  ...
  const phrase=generatePhrasePlan(request.measures,rng); const harmonyResult=generateHarmony(context,meter,request.measures,request.harmony.progressionVocabulary,request.harmony.chordVocabulary,rng);
